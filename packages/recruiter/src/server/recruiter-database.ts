...
function getAuroraConfig(): AuroraConfig {
  ...
  if (missingKeys.length > 0 || isProvisioningHost(process.env.PGHOST)) {
    ...
    throw new Error(
      `RecAI recruiter auth is not connected to Aurora PostgreSQL yet. ${missingDetails} Wait for the AWS for Vercel resource to finish provisioning, then redeploy and run \`vercel env pull\` locally if needed.`,
    );
  }
...
export function isRecruiterDatabaseConfigured() {
  return (
